...
class LQGTDataset(data.Dataset):
    ...
    def __init__(self, opt, mode=None):
        ...
        if opt["data_type"] == "lmdb":
            ...
            self.LR_paths = util.get_image_paths(
                opt["data_type"], opt[mode]["dataroot_LQ"]
            )  # LR list
            self.GT_paths = util.get_image_paths(
                opt["data_type"], opt[mode]["dataroot_GT"]
            )  # GT list
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.opt["phase"] == "train" and 'sidd' in self.mode:
            if jpg_augment: 
                img_GT = util.read_img_as_jpg(
        ...
        if self.LR_paths:  # LR exist
            ...
            if self.opt["phase"] == "train" and 'sidd' in self.mode:
                if jpg_augment: 
                    img_LR = util.read_img_as_jpg(self.LR_env, LR_path, resolution)
                else:

...
def compute_PSNR(
    img1, img2, data_range, trunc_min, trunc_max, norm_range_min, norm_range_max
):
    shape_ = img1.shape[-1]
    # print(img1.shape)
    C = img1.size(0)
    ...
    if data_range != 1.0:
        img1 = trunc(
            denormalize_(
                img1.view(shape_, shape_).cpu().detach(), norm_range_max, norm_range_min
            ),
            trunc_min,
            trunc_max,
        )
        img2 = trunc(
            denormalize_(
                img2.view(shape_, shape_).cpu().detach(), norm_range_max, norm_range_min
            ),
            trunc_min,
            trunc_max,
        )

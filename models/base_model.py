...
class BaseModel:
    ...
    def load_network(self, load_path, network, strict=True):
        ...
        # print("loading keys : ", load_net.keys())
        # load_net = torch.load(
        ...
        if "model" in load_net.keys():
            load_net = load_net["model"]
        if "model_weight" in load_net.keys():
            load_net = load_net["model_weight"]["denoiser"]
        ...
        for k, v in load_net_clean.items():
            if k.startswith("base_net."):
                load_net_clean2[k[9:]] = v
            else:
                load_net_clean2[k] = v

        load_net_clean = load_net_clean2
